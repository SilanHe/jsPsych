...
jsPsych.plugins.fullscreen = (function () {
  ...
  plugin.trial = function (display_element, trial) {
    ...
    const keyboardNotAllowed = typeof Element !== 'undefined' && 'ALLOW_KEYBOARD_INPUT' in Element;
    if (keyboardNotAllowed) {
      // This is Safari, and keyboard events will be disabled. Don't allow fullscreen here.
      // do something else?
      endTrial();
    } else if (trial.fullscreen_mode) {
      display_element.innerHTML = `<div class="display_text">${trial.message}<button id="jspsych-fullscreen-btn" class="jspsych-btn">${trial.button_label}</button></div>`;
      const listener = display_element.querySelector('#jspsych-fullscreen-btn').addEventListener('click', () => {
